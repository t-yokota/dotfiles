...
load_branch_surfaces() {
    ...
    local found=0
    local before_count=${#MANAGED_SURFACES[@]}

    # Surface definition scripts print tab-separated "surface" rows.
    branch=$(git branch --show-current 2>/dev/null || true)
    if [ -z "$branch" ]; then
        log_substep "Skip: no current git branch detected"
        return 0
    fi
    ...
    if [ "$found" -eq 0 ]; then
        log_substep "Skip: no surface definition scripts found"
    elif [ "${#MANAGED_SURFACES[@]}" -eq "$before_count" ]; then
        log_substep "Skip: no managed dotfile surfaces defined for $branch"
    fi
}
...
run_branch_install_checks() {
    ...
    if [ "$found" -eq 0 ]; then
        log_substep "Skip: no branch-specific install checks found"
    fi
...
cleanup_all_managed_surfaces() {
    ...
    if [ "${#MANAGED_SURFACES[@]}" -eq 0 ]; then
        log_substep "Skip: no managed dotfile surfaces defined"
        return 0
    fi
...
link_all_managed_surfaces() {
    ...
    if [ "${#MANAGED_SURFACES[@]}" -eq 0 ]; then
        log_step "Skip: no managed dotfile surfaces defined"
        return 0
    fi
